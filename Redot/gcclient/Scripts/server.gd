...
func _on_button_pressed() -> void:
	...
	if len($CanvasLayer/ColorRect/name.text) > 0:
		randomize()
...
func _input(event: InputEvent) -> void:
	if Input.is_action_just_pressed("e"):
		#$"Maps/1/Cardis/collision/StaticBody3D/CollisionShape3D".disabled = true
		#$"Maps/1".hide()
		
		# Player Teleport Signal-> Call
		Global.emit_signal("position_update", "ww", Vector3(0,0,13.618))
		pass
